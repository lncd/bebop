...
== Description ==
Bebop is a BuddyPress plugin which allows you to connect to other social media services such as Twitter, Slideshare, Youtube and Vimeo. Bebop is an aggregator, pulling content from other web services into your BuddyPress activity timeline.
It will also pull in content from RSS feeds, so will work with sites like Tumblr, WordPress and Blogger. Bebop gives you control over exactly what item of content appears in your activity stream and works on WordPress network installs, too.

Bebop was developed by the University of Lincoln for a JISC-funded 'Open Educational Resources' project, to allow teachers to aggregate teaching resources they have shared on third-party sites into their BuddyPress profile.
BuddyPress is being used at the University of Lincoln to power staff and student profiles.
...
* Introduced a secondary cron which scans a new batabase table every 15 seconds to detect feeds which are due an initial import. This means the default cron can be ran less often, while new users/feeds can still import content.
...
* Improved RSS atom compatability.
...
* Seperated admin functions from normal functions to increase performance.
* Placed buttons in the admin general settings which can automatically fire a cron. This could aid problem solving with users if they cannet get content to import.
...
Bug Fixes:
* A bug was fixed which made the OER filter tab dislay the wrong text.
