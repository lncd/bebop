...
<div id='bebop_admin_container'>
	<div class='postbox center_margin margin-bottom_22px'>
		...
		<div class="inside">
			Settings required for twitter syncronisation.
		</div>
	</div>
	<form class='bebop_admin_form' method='post'>
		<fieldset>
			<span class='header'>Twitter Settings</span>
	...
	if ( count( $user_metas ) > 0 ) {
		...
		<table class='bebop_settings_table'>
			...
			foreach ( $user_metas as $user ) {
				...
				echo '<tr>
					...
					<td><a href='?page=bebop_oer_providers&provider=twitter&reset_user_id=" . bebop_tables::sanitise_element( $user->user_id ) . "'>Reset User</a></td>
				</tr>";
			}
		?>
		<!-- <End bebop_table -->
		...
		echo 'No users found for the Twitter extension.';
	}
	?>
