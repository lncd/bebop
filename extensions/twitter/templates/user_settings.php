...
if( isset( $_GET['reset'] ) ) {
	if ( $_GET['reset']  == 'true' ) {
	    bebop_tables::remove_user_meta($bp->loggedin_user->id, 'twitter_token');
		bebop_tables::remove_user_meta($bp->loggedin_user->id, 'twitter_token_temp');
	    bebop_tables::remove_user_meta($bp->loggedin_user->id, 'twitter_tokensecret');
	    bebop_tables::remove_user_meta($bp->loggedin_user->id, 'twitter_tokensecret_temp');
...
if ( isset( $_GET['oauth_token'] ) ) {
    ...
    $OAuth->setRequestTokenUrl('http://api..com/oauth/request_token');
    $OAuth->setAccessTokenUrl('http://api..com/oauth/access_token');
    $OAuth->setAuthorizeUrl('https://api..com/oauth/authorize');
    ...
    $OAuth->setRequestToken(bebop_tables::get_user_meta_value($bp->loggedin_user->id,'twitter_token_temp'));
    $OAuth->setRequestTokenSecret(bebop_tables::get_user_meta_value($bp->loggedin_user->id,'twitter_tokensecret_temp'));
    
    $accessToken = $OAuth->accessToken();
   
    bebop_tables::update_user_meta($bp->loggedin_user->id,'twitter_token',''.$accessToken['oauth_token'].'');
    bebop_tables::update_user_meta($bp->loggedin_user->id,'twitter_tokensecret',''.$accessToken['oauth_token_secret'].'');
...
if ( bebop_tables::get_user_meta_value($bp->loggedin_user->id, 'twitter_token') ) {
	...
	Before you can begin using  with this site you must authorize on  by clicking the link below.<br/><br/>';
	...
	bebop_tables::update_user_meta($bp->loggedin_user->id,'twitter_token_temp','' . $requestToken['oauth_token'].'');
	bebop_tables::update_user_meta($bp->loggedin_user->id,'twitter_tokensecret_temp','' . $requestToken['oauth_token_secret'].'');
