...
if ( ( bebop_tables::get_option_value( 'bebop_' . $extension['name'] . '_provider' ) == 'on') && ( bebop_tables::get_option_value( 'bebop_' . $extension['name'] . '_consumer_key' ) ) ) {
	if ( ( bebop_tables::get_user_meta_value( $bp->loggedin_user->id, 'bebop_' . $extension['name'] . '_oauth_token' ) ) && ( bebop_tables::check_user_meta_exists( $bp->loggedin_user->id, 'bebop_' . $extension['name'] . '_username' ) ) ) {
		...
		<div class="button_container"><input class="button-secondary" type="submit" id="submit" name="submit" value="Save Changes"></div>';
		...
		if ( bebop_tables::get_user_meta_value( $bp->loggedin_user->id, 'bebop_' . $extension['name'] . '_oauth_token' ) ) {
			echo '<div class="button_container"><a class="button-secondary" href="' . $bp->loggedin_user->domain . bp_current_component() . '/' . bp_current_action() . '/' . $extension['name'] . '?reset=true">'; _e(' Remove Authorisation', 'bebop'); echo '</a></div>';
			echo '<div class="clear_both"></div>';
		}
		echo '</form>';
	}
		...
		if ( ! isset( $_REQUEST['code'] ) ) {
			...
			echo '<div class="button_container"><a class="button-secondary" href="' . $redirectUrl . '">'. __(' Start Authorisation', 'bebop') . '</a></div>';
			echo '<div class="clear_both"></div>';
		}
